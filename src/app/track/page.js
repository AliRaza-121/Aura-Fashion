...
function TrackOrderContent() {
  ...
  const handleSearch = (e) => {
    ...
    if (orderId.trim()) {
      ...
      window.history.pushState(null, '', `?id=${orderId.trim()}`);
    }
  };
...
export default function TrackOrder() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Clock className="animate-spin text-black" size={32} /></div>}>
      <TrackOrderContent />
    </Suspense>
  );
}
